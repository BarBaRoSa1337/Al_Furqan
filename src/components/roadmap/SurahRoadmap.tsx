...
export default function SurahRoadmap({ items, onSelectSurah, onRefresh, refreshing = false, header, direction = 'ltr' }: SurahRoadmapProps) {
  ...
  const renderItem = useCallback(({ item, index }: { item: SurahRoadmapItem; index: number }) => {
    ...
    const nodeLeft = nameSide === 'right' ? pathX - NODE_SIZE / 2 : pathX - NODE_WIDTH + NODE_SIZE / 2;
    return (
      <View style={styles.row}>
        ...
        <View style={[styles.node, { left: Math.max(0, Math.min(nodeLeft, width - NODE_WIDTH)) }]}>
          <SurahRoadmapNode arabicName={item.arabicName} direction={direction} englishName={item.englishName} id={item.id} nameSide={nameSide} onPress={onSelectSurah} state={item.state} />
        </View>
      </View>
    );
  }, [direction, items.length, onSelectSurah, width]);
