...
export default function SurahRoadmapCard({ surah, completed, total, nextTitle, onPress }: SurahRoadmapCardProps) {
  const complete = total > 0 && completed === total;
  const accessibilityLabel = `${surah.transliteratedName}, ${completed} of ${total} lessons complete${nextTitle ? `, next ${nextTitle}` : ''}`;
  return (
    <Pressable
    ...
    >
      ...
      <View style={styles.copy}>
        <Text style={styles.eyebrow}>{complete ? 'SURAH COMPLETE' : 'THEMATIC SURAH PATH'}</Text>
        ...
        <Text numberOfLines={1} style={styles.next}>{complete ? 'Review the Surah' : `Next: ${nextTitle ?? 'Begin'}`}</Text>
      </View>
